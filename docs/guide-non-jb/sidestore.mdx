...
1. Un PC Windows/Mac con AltServer
    * Se hai linux segui [questa guida](https://docs.sidestore.io/it/docs/installation/linux/) 
3. Un Apple ID
4. Un dispositivo con iOS/iPadOS 14 o superiore (con un [passcode](https://support.apple.com/119586))
...
5. Ora dal tuo iDevice:
    * Vai in `Impostazioni` > `Generali` > `VPN e Gestione Dispositivo` > `<il tuo Apple ID>` e clicca "Autorizza"
    * Vai in `Impostazioni` > `Privacy e Sicurezza` e attiva la "Modalità Sviluppo" 
...
:::warning
L'installazione e il refresh delle app può avvenire **esclusivamente** sotto rete Wi-Fi
:::

1. Sull'iPhone/iPad, ~scarica [StosVPN](https://apps.apple.com/app/stosvpn/id6744003051) dall'AppStore~
    * StosVPN purtroppo è stato rimosso dall'AppStore, non sappiamo quando e se verrà ricaricato, dunque per poter far funzionare SideStore installiamo StikDebug:
        * Installa [AltStore PAL](https://altstore.io/download) direttamente dall'iPhone
        * Segui i passaggi, apri AltStore PAL e vai su Sources, premi il `+` in alto a sinistra e aggiungi "Stephen's Apps"
        * Installa StikDebug e quando ti viene richiesto, passagli il pairing file (lo stesso che hai generato per SideStore)
...
Per usare StikDebug è necessario avere iOS/iPadOS 17.4 o successive.
:::

1. Installa [StikDebug](https://apps.apple.com/app/stikdebug/id6744045754) dall'App Store
    * Può capitare che StikDebug venga rimossa dall'App Store, in tal caso puoi installarla tramite [AltStore PAL](https://altstore.io/download) dalla repo `https://stikdebug.xyz/apps.json`
